        ...
        <div className="min-h-screen" style={{ background: 'linear-gradient(180deg, #f9fafb 0%, #f1f5f9 100%)' }}>
            ...
            <main className="pt-32 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
                ...
                <div className="flex justify-between items-start mb-10">
                    <div>
                        <h1 className="text-3xl font-display font-semibold text-slate-900 mb-1">Merchant Dashboard</h1>
                        <p className="text-slate-400 text-sm">Manage your surplus listings and incoming orders.</p>
                    </div>
                    <Button
                ...
                {/* New Listing Form */}
                {isListing && (
                    <div className="bg-white rounded-2xl border border-slate-100 p-8 mb-10 animate-in slide-in-from-top-4 duration-300" style={{ boxShadow: '0 10px 30px rgba(0,0,0,0.08)' }}>
                ...
                {/* Active Listings */}
                <div className="mb-12">
                    <h2 className="text-xl font-display font-semibold text-slate-900 mb-5">Active Listings</h2>
                    {loading ? (
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                            {[1, 2, 3].map((i) => (
                                <div key={i} className="bg-white rounded-2xl border border-slate-100 overflow-hidden" style={{ boxShadow: '0 10px 30px rgba(0,0,0,0.08)' }}>
                        ...
                        <div className="bg-white rounded-2xl border border-slate-100 py-16 text-center" style={{ boxShadow: '0 10px 30px rgba(0,0,0,0.08)' }}>
                            ...
                            <p className="text-slate-700 font-semibold mb-1">No active listings</p>
                            <p className="text-slate-400 text-sm">Create your first box listing to start rescuing food.</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                            {items.map((item, idx) => (
                                <div key={item.itemID || idx} className="bg-white rounded-2xl border border-slate-100 overflow-hidden hover:-translate-y-0.5 transition-all duration-200 animate-fade-up" style={{ animationDelay: `${idx * 0.06}s`, boxShadow: '0 10px 30px rgba(0,0,0,0.08)' }}>
                                    ...
                                    <div className="p-5">
                                        <div className="flex items-start justify-between mb-3">
                                            <div>
                                                <p className="text-xs text-slate-400 font-medium mb-0.5">ID: {item.itemID}</p>
                                            ...
                                            {item.quantity > 0 ? (
                                                ...
                                                <span className="text-xs font-semibold px-2 py-1 rounded-full bg-red-50 text-red-600 shrink-0">Sold Out</span>
                                            )}
                                        </div>
                                        ...
                                        <div className="flex gap-2 pt-3 border-t border-slate-50">
                                            <button onClick={() => handleEditListing(item)} className="flex-1 text-xs font-semibold text-slate-500 hover:text-slate-900 bg-slate-50 hover:bg-slate-100 rounded-lg py-2 transition-colors">
                                                Edit
                                            </button>
                                            <button onClick={() => confirmRemoveListing(item)} className="flex-1 text-xs font-semibold text-red-500 hover:text-red-700 bg-red-50 hover:bg-red-100 rounded-lg py-2 transition-colors">
                                                Remove
                                            </button>
                                        </div>
                                    </div>
                ...
                <div>
                    ...
                    {ordersLoading ? (
                        <div className="space-y-3">
                            {[1, 2].map((i) => (
                                <div key={i} className="bg-white rounded-2xl border border-slate-100 p-5" style={{ boxShadow: '0 10px 30px rgba(0,0,0,0.08)' }}>
            ...
            {showRemoveModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-8 animate-in slide-in-from-bottom-4 duration-300">
                        <h3 className="text-xl font-display font-semibold text-slate-900 mb-2">Remove Listing</h3>
                        <p className="text-slate-500 text-sm mb-6">
                            Are you sure you want to remove "<span className="font-semibold text-slate-800">{itemToRemove?.name}</span>"? This cannot be undone.
                        </p>
